...
def column_avg(table: dict[str, list[str]], columns: list[str]) -> dict[str, list[float]]:
    """Return average values of a certain column."""
    result: dict[str, list[float]] = {}
    int_table = {}
    for column in columns:
        values_list: list[str] = table[column]
        for i in range(0, len(values_list)):
            values_list[i] = int(values_list[i])  # type: ignore
        int_table[column] = values_list
    for column in columns:
        num_rows = len(table[column])
        result[column] = [sum(int_table[column]) / num_rows]
    return(result)